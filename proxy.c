...
#define SERVERPORT 8888   //The port on which to send data
...
int main(void)
{
    ...
    si_me.sin_port = htons(SENDERPORT);
    ...
    while(1)
    {
        ...
        //send the message
        if (sendto(skt, message, strlen(message) , 0 , (struct sockaddr *) &si_other, slen)==-1)
        {
            die("sendto()");
        }
        ...
        if (sendto(s, buf1, recv_len, 0, (struct sockaddr*) &si_other1, slen1) == -1)
        {
            die("sendto()");
        }
